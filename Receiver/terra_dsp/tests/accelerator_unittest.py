...
from terra_dsp.score_functions import max_mean_ratio_cpu, max_mean_ratio_gpu, get_gpu_score_kernel
...
class TestAccelerators(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.score_kernel = get_gpu_score_kernel(self.ctx)
    ...
    def testsucceed_featureextractor(self):
        rng = np.random.default_rng(11)
        samples = (rng.standard_normal(10000) + 1j * rng.standard_normal(10000))/10
        scores_cpu = max_mean_ratio_cpu(samples)

        start = time.time()
        scores_gpu = max_mean_ratio_gpu(samples, self.ctx, self.queue, self.score_kernel)
        end = time.time()
        print(f'GPU Ran in {end-start}s')
        self.assertTrue(np.allclose(scores_cpu, scores_gpu, rtol=1e-5, atol=1e-5))
