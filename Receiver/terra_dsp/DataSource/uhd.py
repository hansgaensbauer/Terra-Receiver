...
class USRPSource(DataSource):
    ...
    def _rx_process(self, stop_flag, shm_names, kwargs):
        ...
        sdr.set_rx_rate(self.fs, 0)
        sdr.set_rx_freq(uhd.libpyuhd.types.tune_request(self.fc), 0)
        if("gain" in kwargs.keys()):
            sdr.set_rx_gain(kwargs["gain"], 0)

        metadata = uhd.types.RXMetadata()
        streamer = sdr.get_rx_stream(st_args)

        recv_buffer = np.zeros((1, self.buffer_length), dtype=np.complex64)
        num_samples = 0
        ...
        while(1):
            while(stop_flag.is_set()):
                for i in range(self.chunk_length//self.buffer_length):
                    ...
                    arrays[shared_memory_index][i*self.buffer_length:(i+1)*self.buffer_length] = recv_buffer[0]
